<x-guest-layout>
    <x-authentication-card>
        ...
        <form method="POST" action="{{ route('userlogin') }}">
            ...
            {{--these error divs are not neccessary because of the x-authentication card but i added them anyway--}}

            @if ($errors->has('email'))
            <div class="error text-red-600">{{ $errors->first('email') }}</div>
            @endif
            ...
            <div class="error text-red-600">{{ $errors->first('password') }}</div>
            @endif
